"""
preprocess_co_emissions.jl

Convert the CMIP/CEDS anthropogenic CO emissions file to a format compatible
with ClimaUtilities.TimeVaryingInputs / InterpolationsRegridder.

Input:  CO-em-anthro_CMIP_CEDS_2020.nc
          - dims:    (lon=720, lat=360, time=12), 0.5° resolution
          - sectors: CO_agr, CO_ene, CO_ind, CO_tra, CO_rco, CO_slv, CO_wst, CO_shp
          - units:   kg m⁻² s⁻¹
          - time:    "days since 2020-01-01" (monthly, Jan–Dec)
          - _FillValue = 1e20 (land-only sectors → 0 over ocean)

Output: CO-em-anthro_CMIP_CEDS_2020_climaatmos.nc
          - dims:    (lon=720, lat=360, time=12)  — no _FillValue on CO_total
          - variable: CO_total  [kg m⁻² s⁻¹]   (sum of all sectors; missing→0)
          - time:    relabelled to "hours since 1985-01-01" to match simulation
                     reference date; month mid-points (15th of each month)

Usage:
    julia --project preprocess_co_emissions.jl [input_file] [output_file]
"""
...
# ── paths ─────────────────────────────────────────────────────────────────────
input_file  = length(ARGS) >= 1 ? ARGS[1] : "CO-em-anthro_CMIP_CEDS_2020.nc"
output_file = length(ARGS) >= 2 ? ARGS[2] : "CO-em-anthro_CMIP_CEDS_2020_climaatmos.nc"
...
NCDataset(input_file, "r") do ds_in
    ...
    co_total = zeros(Float32, nlon, nlat, nt)   # (lon, lat, time)
    ...
    for sector in SECTORS
        ...
        for k in 1:nt, j in 1:nlat, i in 1:nlon
            v = raw[i, j, k]
            co_total[i, j, k] += ismissing(v) ? 0f0 : Float32(v)
        end
        println("  added $sector  (max = $(maximum(skipmissing(raw))))")
    end
    println("CO_total range: $(minimum(co_total)) – $(maximum(co_total)) kg m⁻² s⁻¹")
    ...
    NCDataset(output_file, "c") do ds_out
        ...
        # CO total — no _FillValue: zero is a valid emission (ocean grid cells)
        v_CO = defVar(ds_out, "CO_total", Float32, ("lon", "lat", "time"))
        v_CO[:, :, :] = co_total
        v_CO.attrib["units"]     = "kg m-2 s-1"
        v_CO.attrib["long_name"] = "Total anthropogenic CO emission (all CEDS sectors)"
    end

    println("Done. Global total (Jan): $(sum(co_total[:,:,1]) * 0.5^2 * (π/180)^2 * 6.371e6^2) kg s⁻¹")
end
